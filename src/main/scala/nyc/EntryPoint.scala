...
object EntryPoint extends App {
  ...
  override def main(args: Array[String]) {
    val spark = SparkSession
      .builder()
      .appName("test") // YOU NEED TO CHANGE YOUR GROUP NAME
      .config("spark.some.config.option", "some-value").master("spark://10.212.26.222:7077")
      .getOrCreate()
  ...
  private def queryLoader(spark: SparkSession, queryName: String, queryParams: String, outputPath: String) {
    ...
    if (queryName.equalsIgnoreCase("hotcellanalysis")) {
      if (queryParam.length != 1) throw new ArrayIndexOutOfBoundsException(" Query " + queryName + " needs 1 parameters but you entered " + queryParam.length)
      ...
      if (queryParam.length != 2) throw new ArrayIndexOutOfBoundsException(" Query " + queryName + " needs 2 parameters but you entered " + queryParam.length)
      HotareaAnalysis.runHotZoneAnalysis(spark, queryParam(0), queryParam(1)).write.mode(SaveMode.Overwrite).csv(outputPath)
    }
    else {
